...
namespace UTJ.ProfilerReader.BinaryData.Stats
{
    public partial class MemoryStats
    {
        internal void Read_0x20161226(System.IO.Stream stream)
        {
            ...
            this.bytesReservedVideo = ProfilerLogUtil.ReadInt(stream) * 1024;
            ...
            profilerMemUsed = ProfilerLogUtil.ReadInt(stream);

            profilerNumAllocations = ProfilerLogUtil.ReadInt(stream);

            frameGCAllocCount = ProfilerLogUtil.ReadInt(stream);
            frameGCAllocBytes = ProfilerLogUtil.ReadInt(stream);
